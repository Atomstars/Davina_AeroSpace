...
export default function TacticalEarth({ globeElevation = -4.5, showNavigation = false, pointer, routeDefinition }: TacticalEarthProps) {
    ...
    const orbitLines = useMemo(() => {
        ...
        return {
            inner: createOrbitGeometry(8.0),
            middle: createOrbitGeometry(8.8),
        };
    }, []);
    ...
    const orbitMeshes = useMemo(() => {
        ...
        return {
            inner: createOrbitLine(orbitLines.inner, 0x22d3ee, 0.28),
            middle: createOrbitLine(orbitLines.middle, 0x38bdf8, 0.22),
        };
    }, [orbitLines]);
    ...
    useFrame((_, delta) => {
        ...
        orbitLineRefs.current.forEach((line, idx) => {
            ...
            line.material.opacity = 0.16 + 0.08 * Math.sin(Date.now() * 0.002 + idx);
        });
        ...
        >
            ...
            <mesh ref={earthRef}>
                ...
                <meshStandardMaterial
                    map={dayMap} 
                    color="#888888" // Darkens the Earth so it's moody, but keeps oceans blue!
                    emissiveMap={nightMap} 
                    emissive={new THREE.Color("#ffcc66")} // Golden city lights
                    emissiveIntensity={2.0} 
                    bumpMap={bumpMap} 
                    bumpScale={0.5} // Realistic physical mountains
                    roughness={0.7}
                    metalness={0.2}
                />
            </mesh>
            ...
            <group ref={ringsRef}>
                <group rotation={[Math.PI / 2, 0, 0]}> 
                    <mesh>
                        <torusGeometry args={[8.0, 0.025, 16, 128]} />
                ...
                <group rotation={[0, Math.PI / 2, 0]}> 
                    <mesh>
                        <torusGeometry args={[8.8, 0.025, 16, 128]} />
                ...
                {satelliteNodes.map((node, i) => {
                    const radius = node.orbitIndex === 0 ? 8.0 : 8.8;
                    ...
                    return (
                        <group key={i} rotation={rot} ref={(el) => { if (el) satelliteOrbitRefs.current[i] = el; }}>
                            <group position={[Math.cos(node.angle) * radius, Math.sin(node.angle) * radius, 0]}>
                                <mesh>
                                    <boxGeometry args={[0.16, 0.06, 0.08]} />
                                    <meshStandardMaterial color="#94a3b8" roughness={0.7} metalness={0.12} />
                                </mesh>
                                <mesh position={[0.14, 0, 0]}>
                                    <boxGeometry args={[0.18, 0.01, 0.08]} />
                                    <meshBasicMaterial color="#0ea5e9" transparent opacity={0.45} toneMapped={false} />
                                </mesh>
                                <mesh position={[-0.14, 0, 0]}>
                                    <boxGeometry args={[0.18, 0.01, 0.08]} />
                                    <meshBasicMaterial color="#0ea5e9" transparent opacity={0.45} toneMapped={false} />
